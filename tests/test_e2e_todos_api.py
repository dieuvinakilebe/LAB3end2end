...
def test_e2e_create_read_update_delete_todo():
    ...
    # с увеличенным лимитом, чтобы в выборку попала только что созданная задача
    r_list = requests.get(f"{BASE_URL}/todos", params={"skip": 0, "limit": 100})
    # если в твоём роутере нет skip/limit, можно просто оставить requests.get(f"{BASE_URL}/todos")
